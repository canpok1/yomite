...
- **区切り文字（直後で分割）:** 句点（。）、感嘆符（！）、疑問符（？）、ピリオド（.）、半角感嘆符（!）、半角疑問符（?）、閉じ引用符（」）
  - 区切り文字は直前の文に含める。
...
### 5.1 AI読者シミュレーション
* **一括送信方式:** Geminiに対し、選択された `system_prompt` と全 `Sentences` を一括で送信し、全ステップの読解シミュレーション結果を `[]SimulationStep` のJSON配列で取得する。
* **バックトラック検知:** 指示語が不明瞭な際や、論理の不一致を感じた際に、過去の `SentenceIdx` へ戻る挙動を記録。
* **レート制限対応:** API呼び出し失敗時は指数バックオフによるリトライで対応する。
